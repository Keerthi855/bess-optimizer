...
if uploaded is None:
    ...
    if 'result' in st.session_state:
        ...
        k3.metric("Install Decision", "✅ Yes" if res['y_installed'] else "❌ No",
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
            "📈 Dispatch", "🔋 SOC & Price", "📅 Monthly", "💰 Economics", "🌡️ Price Heatmap"
        ])
        ...
        vc4.metric("Annual Cycles", f"{ann_cycles:.0f}",
                   delta="✅ Normal" if 50 < ann_cycles < 1000 else "⚠️ Check",
                   delta_color="off")
...
'@ | Set-Content -Path streamlit_app.py -Encoding UTF8
