...
def optimize():
    try:
        ...
        # ── Battery power cap ────────────────────────────────────
        # Battery discharge is capped at solar installed capacity (kWp)
        # This represents the inverter / grid connection shared with solar
        p_hard_cap   = solar_kw   # kW — absolute discharge limit
        ...
        m.P  = pyo.Var(bounds=(0, min(e_max_kwh * c_rate, p_hard_cap)), domain=pyo.NonNegativeReals)  # kW power
        ...
        m.ch_grd  = pyo.Var(m.T,  domain=pyo.NonNegativeReals)  # charge from grid kW
        ...
        m.c_bal = pyo.Constraint(m.T, rule=lambda mo, t:
            ...
            == load[t] + mo.ch_sol[t] + mo.ch_grd[t] + mo.exp[t])
        ...
        # ch_sol[t] + ch_grd[t] <= P * u[t]   → charge only when u=1
        ...
        p_M = min(e_max_kwh * c_rate, p_hard_cap)  # tightest possible bound on P
        ...
        total_ch_grd     = sum(pyo.value(m.ch_grd[t]) for t in range(N))
        ...
        for t in range(N):
            hourly.append({
                ...
                'ch_grid'      : round(pyo.value(m.ch_grd[t]), 2),
        ...
        return jsonify({
            ...
            'N_HOURS'           : N,
            'hourly'            : hourly,
            # Raw inputs (echoed back so /report can reproduce validation & payback checks)
...
def report():
    """Takes the same JSON that /optimize returns (sent back by the browser)
    and builds a full Word report: KPI table, combined dashboard, individual
    charts, validation checks, and an investment verdict."""
    try:
